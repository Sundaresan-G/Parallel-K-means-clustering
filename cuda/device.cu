...
int num_points, int dimensions, int knumber, float *centroids,\
float *d_centroids, int *d_clusterSize, int *h_clusterSize, \
float *h_newCentroids, int *d_clust_assn){
    ...
    cudaMemcpy(h_clusterSize, d_clusterSize, \
        knumber*sizeof(int), cudaMemcpyDeviceToHost);

    // To store host sum
    float *h_sum = new float[knumber*dimensions];
    ...
    for(int i=0; i<knumber*dimensions; i++){
        int clusterIndex = i / dimensions;
        if(h_clusterSize[clusterIndex]!=0){
            h_newCentroids[i] = h_sum[i]/h_clusterSize[clusterIndex];
        } else {
            h_newCentroids[i] = centroids[i];
        }
    }
...
const int dimensions, float *centroids, int *clusterSize, const int knumber, const int max_iter_count, \
const float epsilon, float *finalNormChange){
    ...
    do {
        ...
        knumber, centroids, d_centroids, d_clusterSize, clusterSize, newCentroids, d_clust_assn);
    ...
    *finalNormChange = normCentroidChange;
